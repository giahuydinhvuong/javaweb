				...
				<ul class="nav nav-list">
					<c:forEach var="item" items="${ categorys }">
						...
						<li> 
							...
							<span class="badge badge-warning pull-right" style="line-height:18px;">${ TotalQuantyCart }</span>
							</strong></a></li>
				</ul>
			</div>
...
-->
	...
	-->
		<div class="well well-small">
		  <h3><a class="btn btn-mini pull-right" href="products.html" title="View more">VIew More<span class="icon-plus"></span>
		  </a> sản phẩm nổi bật </h3>
		  <hr class="soften"/>
		  <div class="row-fluid">
		  <c:if test="${ highproducts.size() > 0 }">
					<ul class="thumbnails">
					
					<c:forEach var="item" items="${ highproducts }" varStatus="loop">
						<li class="span4">
							<div class="thumbnail">
								<a class="zoomTool" href="<c:url value="/chi-tiet-san-pham/${ item.id_product }"/>"
						...
						<c:if test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == highproducts.size() }">
							</ul>
							<c:if test="${ (loop.index + 1) < highproducts.size() }">
								<ul class="thumbnails">
							</c:if>
						</c:if>
